...
- `.github/workflows/release.yml` builds macOS (`arm64` and `x64`), Linux (`x64`), and Windows (`x64` and `arm64`) desktop artifacts from a single `v*.*.*` tag and publishes one GitHub release.
