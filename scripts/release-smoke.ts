...
try {
  ...
  for (const entry of NodeFS.readdirSync(releaseAssetsDirectory)) {
    if (!entry.endsWith("-win-x64.yml") || entry.startsWith("builder-debug-")) {
      continue;
    }

    const x64Manifest = NodePath.resolve(releaseAssetsDirectory, entry);
    const arm64Manifest = NodePath.resolve(
      releaseAssetsDirectory,
      entry.replace(/-x64\.yml$/, "-arm64.yml"),
    );
    const outputManifest = NodePath.resolve(
      releaseAssetsDirectory,
      entry.replace(/-win-x64\.yml$/, ".yml"),
    );
